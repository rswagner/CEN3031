angular.module('listings').controller('ListingsController', ['$scope', '$location', '$stateParams', '$state', 'Listings',
  function($scope, $location, $stateParams, $state, Listings){
    ...
    $scope.update = function(isValid) {
       ...
       */
       ...
       updatedListing.code.toUpperCase();
       console.log(updatedListing.code);


       Listings.update(id,updatedListing).then(function(response) {
                //if the object is successfully saved redirect back to the list page
  ...
  // Listings.collection("entries").updateOne(myquery, newvalues, function(err, res) {
  //   if (err) throw err;
...
//        Listings.update(
//     { _id : listing._id },
//     { $set : { name:updatedListing.name } },
//
// ).then(function(response) {
//            $state.go('listings.list', { successMessage: 'Listing has been updated!' });
//          }, function(error) {
//            $scope.error = 'Could not add listing. Error: ' + error;
//          });

      //  Listings.update({'_id':ObjectID(listing)},
      //    {$set: {'name': updatedListing.name}, {'code':updatedListing.code},
    ...
    $scope.remove = function(listing) {
       ...
       */
       Listings.delete(listing._id).then(function(response) {
                 ...
                 $scope.error = 'Could not delete listing. Error: ' + error;
               });
    };
